...
const addStore = async ({ storeName }:StoreName) => {
...
const createStoreData = async () => {
  ...
  const existingStoreNames: string[] = stores.map((store) => store.storeName).filter((name): name is string => !!name);
  const newStoreNames: string[] = StoresData.map((data: IStore) => data.storeName).filter(
    (name: string) => !existingStoreNames.includes(name)
  );
